...
#  include <nuttx/timers/timer.h>
  #include "stm32_tim.h"  // Để có prototype stm32_timer_initialize
#endif
...
#ifdef CONFIG_STM32_ADC
/* ADC1 Channel 1 corresponds to PA1 */
#define ADC1_NCHANNELS  1

/* ADC channel list: ADC1 */
static const uint8_t g_chanlist[ADC1_NCHANNELS] = 
{
  1  /* ADC1 Channel 1 (PA1) */
};
...
int stm32_bringup(void)
{
  ...
  /* Khởi tạo I2C1 */
  i2c = stm32_i2cbus_initialize(2); // Port I2C1
  if (i2c == NULL)
  {
      /* Lỗi khởi tạo I2C1 */
  ...
  ainfo("ADC1 initialized successfully\n");
#endif

#ifdef CONFIG_TIMER
  /* Register timer 1 drivers */
  ret = stm32_timer_initialize("/dev/timer1", 1);
  if (ret < 0)
    {
...
#ifdef CONFIG_STM32_PWM
  /* Register timer 2 pwm drivers */
  struct pwm_lowerhalf_s *pwm2;

  /* Khởi tạo PWM với TIM1 */
  pwm2 = stm32_pwminitialize(2);
  if (!pwm2)
    {
      printf("Failed to initialize PWM on TIM2\n");
      return -ENODEV;
    }

  /* Đăng ký device node /dev/pwm0 */
  ret = pwm_register("/dev/pwm2", pwm2);
  if (ret < 0)
    {
      printf("Failed to register PWM device: %d\n", ret);
      kmm_free(pwm2);
      return ret;
    }
#endif
