...
#define GPIO_SPI1_CS_ICM (GPIO_OUTPUT | GPIO_PUSHPULL | GPIO_SPEED_50MHz | \
...
void board_spi1_sensor_select(uint8_t devid, bool selected)
{
    switch (devid)
    {
        case 0:  /* ICM42688P #0 */
            stm32_gpiowrite(GPIO_SPI1_CS0_ICM, !selected);
            break;

        case 1:  /* ICM42688P #1 */
            stm32_gpiowrite(GPIO_SPI1_CS1_ICM, !selected);
            break;

        case 2:  /* ICM42688P #2 */
            stm32_gpiowrite(GPIO_SPI1_CS2_ICM, !selected);
            break;

        case 3:  /* ICM42688P #3 */
            stm32_gpiowrite(GPIO_SPI1_CS3_ICM, !selected);
            break;

        /* BMM150 removed - now uses I2C1 */
