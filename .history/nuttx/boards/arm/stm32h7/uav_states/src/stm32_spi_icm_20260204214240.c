...
/* Wrapper thiết bị cho mỗi cảm biến */
...
{
  int i;
  static bool cs_pins_initialized = false;

  _info("Starting SPI1 initialization for ICM42688P\n");

  /* Ensure CS GPIOs are configured before any SPI transactions.
   * This is normally done in board bring-up, but sensors_app can also
   * lazy-init SPI, so do it here defensively.
   */

  if (!cs_pins_initialized)
    {
      stm32_spidev_icm_initialize();
      cs_pins_initialized = true;
    }
...
{
  /* Lazy init in case bringup was not executed */

  if (g_spi1_icm == NULL)
    {
      int ret = board_spi1_icm_initialize();
      if (ret < 0)
        {
          _err("ERROR: SPI1 not initialized (ret=%d)\n", ret);
          return NULL;
        }
    }
