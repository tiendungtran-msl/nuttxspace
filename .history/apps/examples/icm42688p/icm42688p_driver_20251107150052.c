...
int icm42688p_initialize(icm42688p_dev_t *dev)
{
    ...
    /* Open SPI device */
    dev->spi_fd = icm42688p_spi_init(ICM42688P_SPI_BUS, dev->sensor_id);
    if (dev->spi_fd < 0) {
    ...
    ret = icm42688p_read_register(dev->spi_fd, dev->sensor_id, 
                                   ICM42688P_WHO_AM_I, &who_am_i);
    if (ret < 0) {
...
int icm42688p_reset(icm42688p_dev_t *dev)
{
    ...
    ret = icm42688p_write_register(dev->spi_fd, dev->sensor_id,ICM42688P_DEVICE_CONFIG, 0x01);
    if (ret < 0) {
