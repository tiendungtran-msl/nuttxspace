...
int i2c_manager_init(int bus)
{
  ...
  if (g_i2c_fd < 0)
    {
      syslog(LOG_ERR, "I2C: Failed to open %s: %d\n", devpath, errno);
      return -errno;
    }

  syslog(LOG_INFO, "I2C: Opened %s\n", devpath);

  return OK;
}

/****************************************************************************
 * Name: i2c_manager_deinit
 ****************************************************************************/
