...
/* System state */
bmm150_dev_t g_bmm150_dev;  /* Remove 'static' */
...
static int init_drivers(void)
{
  ...
  ret = bmm150_init(&g_bmm150_dev, IMU_I2C_BUS, IMU_BMM150_I2C_ADDR);
  if (ret < 0)
    {
...
static int calibrate_sensors(bool mag_only)
{
  ...
  ret = bmm150_calibrate(&g_bmm150_dev, IMU_MAG_CALIB_SAMPLES);
  if (ret < 0)
    {
  ...
  if (g_debug_output || g_verbose_output)
    {
      ...
      printf(" | BMM150: reads=%lu errors=%lu", (unsigned long)bmm_reads, (unsigned long)bmm_errors);
