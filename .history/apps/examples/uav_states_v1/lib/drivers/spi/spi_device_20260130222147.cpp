...
extern "C" {
// Low-level STM32 SPI init (fallback, usually already done by board)
struct spi_dev_s *stm32_spibus_initialize(int bus);

// Board-level API for ICM42688P SPI devices (preferred)
...
namespace drivers {
    namespace spi {
        ...
        int Device::init()
        {
            ...
            // For SPI1 with ICM sensors, use board-level wrapper which handles
            // per-sensor CS pin selection properly
            if (_config.bus == 1) {
                // Convert devid to sensor index: SPIDEV_IMU(1)=1 -> id=0, etc.
                uint8_t sensor_id = _config.devid - 1;
                if (sensor_id > 3) {
                    DRIVER_ERR("Invalid ICM sensor ID %d (devid=%d)", sensor_id, _config.devid);
                    return -EINVAL;
                }
                ...
                // For other SPI buses, use standard initialization
                if (!board_has_spi_bus(_config.bus)) {
                    DRIVER_WARN("SPI bus %d not reported by board_has_spi_bus(); attempting init anyway", _config.bus);
                }
