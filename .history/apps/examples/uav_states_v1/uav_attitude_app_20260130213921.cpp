...
// IMPORTANT:
// - UART4 is the NSH console on this board (often /dev/ttyS1 per current .config).
// - UART8 is for external telemetry (GPS/UART8 pins), and should NOT share the console.
// With only UART4+UART8 enabled, UART8 is typically /dev/ttyS0.
static const char* g_telem_device = "/dev/ttyS0";
static bool g_telem_enabled = false;
static bool g_telem_disabled_by_user = false;
static bool g_telemetry_started = false;
...
{
    // Safety: never try to use the current console UART for telemetry.
    // If someone passes the console device path, refuse to avoid breaking minicom.
    if (device == nullptr || strcmp(device, "/dev/console") == 0 || strcmp(device, "/dev/ttyS1") == 0) {
        errno = EINVAL;
        return -1;
    }
    ...
    // Deliberately do NOT call tcsetattr()/cfmakeraw() here.
    // On NuttX, termios settings can be shared at the driver level; if the
    // telemetry path accidentally points to the console UART, this can break NSH.
    return 0;
}
    ...
    while (g_running) {
        ...
        for (int i = 0; i < NUM_IMUS; i++) {
            ...
            if (orb::topic_sensor_imu[i].copy(imu_data)) {
                ...
                euler_msg.roll_deg = euler.roll * 57.2957795f;
                euler_msg.pitch_deg = euler.pitch * 57.2957795f;
                euler_msg.yaw_deg = euler.yaw * 57.2957795f;
                euler_msg.gyro_bias[0] = bias.x;
                euler_msg.gyro_bias[1] = bias.y;
                euler_msg.gyro_bias[2] = bias.z;
                euler_msg.valid = true;
...
static bool start_tasks(void)
{
    ...
    // Telemetry task - low priority (optional)
    if (g_telem_enabled) {
        pthread_attr_init(&attr);
...
static void print_usage(const char* prog)
{
    ...
    printf("  --no-telem   Disable telemetry UART completely (console-only)\n");
...
extern "C" int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--no-telem") == 0) {
            g_telem_disabled_by_user = true;
            continue;
        }
    ...
    printf("  Telemetry: %s\n", g_telem_disabled_by_user ? "(disabled)" : g_telem_device);
    printf("========================================\n\n");
    ...
    // Initialize telemetry UART (optional). This must NOT share the NSH console UART.
    if (!g_telem_disabled_by_user) {
        if (telem_uart_init(g_telem_device) == 0) {
            g_telem_enabled = true;
            printf("Telemetry UART: %s (UART8)\n", g_telem_device);
        } else {
            printf("Telemetry UART: DISABLED (failed to open %s, errno=%d)\n",
                   g_telem_device, errno);
        }
    } else {
        printf("Telemetry UART: DISABLED (--no-telem)\n");
    }
    ...
    if (g_telem_enabled) {
        printf("Connect Python tool to UART8 (host /dev/ttyUSBx).\n\n");
    } else {
        printf("Console-only mode.\n\n");
    }
    
    // Main thread: periodic status + console attitude output (low rate)
    uint32_t last_sensor = 0, last_est = 0, last_telem = 0;
    orb::vehicle_euler_s e0;
    
    while (g_running) {
        sleep(1);  // Status every 1 second
        
        if (!g_running) break;
        
        // Calculate rates
        uint32_t sensor_rate = (g_sensor_count - last_sensor);
        uint32_t est_rate = (g_estimator_count - last_est);
        uint32_t telem_rate = (g_telem_count - last_telem);
        
        last_sensor = g_sensor_count;
        last_est = g_estimator_count;
        last_telem = g_telem_count;

        // Show IMU0 attitude if available
        if (orb::topic_vehicle_euler[0].copy(e0) && e0.valid) {
            printf("[Status] S:%luHz E:%luHz T:%luHz | IMU0 R:%+6.1f P:%+6.1f Y:%+6.1f\n",
            ...
            printf("[Status] S:%luHz E:%luHz T:%luHz | IMU0 (no data yet)\n",
                   (unsigned long)sensor_rate,
                   (unsigned long)est_rate,
                   (unsigned long)telem_rate);
        }
