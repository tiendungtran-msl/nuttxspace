/****************************************************************************
 * apps/examples/adc_reader/voltage_reader_main.c
 *
...
static int read_voltage_from_pa1(int fd)
{
  ...
  printf("🔍 Bắt đầu đọc ADC...\n");

  /* Đọc dữ liệu từ ADC */
  nbytes = read(fd, adc_sample, sizeof(adc_sample));
  
  printf("📊 Đã đọc %ld bytes từ ADC\n", (long)nbytes);
  ...
  if (samples_count > 0)
    {
      ...
      /* Hiển thị kết quả - Sử dụng double cho tương thích */
      double voltage_d = (double)voltage;
      double percent_d = (voltage_d / ADC_VREF) * 100.0;
      
      printf("📊 PA1 - Giá trị thô: %4ld | Điện áp: %d.%03dV | Phần trăm: %d.%d%%\n",
      ...
      if (result < 0)
        {
          ...
          sleep(1);
          continue;  /* Tiếp tục thử thay vì thoát */
        }
