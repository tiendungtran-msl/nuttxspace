...
#else
  /* Fallback implementation */
  gpioinfo("PC13 setpintype %d (STM32 HAL not available)\n", pintype);
  return OK;
#endif
}
...
#else
  /* 
   * Fallback for non-STM32F4 platforms or when STM32 HAL not available
   * Just mark as initialized to avoid repeated calls
   */
  gpioinfo("STM32F4 HAL not available, using simplified GPIO setup\n");
  g_gpio_initialized = true;
  ret = OK;
#endif
