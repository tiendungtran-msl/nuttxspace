...
int i2c_init(i2c_dev_t *dev, uint8_t bus, uint8_t addr, uint32_t frequency)
{
  ...
  dev->addr = addr;
  dev->frequency = (frequency > 0) ? frequency : I2C_DEFAULT_FREQUENCY;
  ...
  /* Cấu hình frequency */
  ret = ioctl(dev->fd, I2CIOC_SETFREQUENCY, 
              (unsigned long)((uintptr_t)&dev->frequency));
  if (ret < 0)
    {
      _err("ERROR: Failed to set I2C frequency: %d\n", errno);
      close(dev->fd);
  ...
  _info("I2C initialized: %s, addr=0x%02X, freq=%u Hz\n",
        dev->devpath, dev->addr, dev->frequency);
