...
/* Register bank select register address (for ICM42688P) */
                  ...
                  size_t length)
{
  ...
  while (retries-- > 0)
  {
    ret = write(dev->fd, txbuffer, length);
    if (ret == (int)length)
    {
      ret = read(dev->fd, rxbuffer, length);
      if (ret == (int)length)
      {
        ret = 0;
        break;
             ...
             uint32_t frequency, int8_t mode)
{
  ...
  dev->bus       = bus;
  dev->devid     = devid;
  dev->frequency = (frequency > 0) ? frequency : SPI_DEFAULT_FREQUENCY;
  dev->mode      = (mode >= 0 && mode <= 3) ? mode : SPI_DEFAULT_MODE;
  dev->nbits     = SPI_DEFAULT_NBITS;
  ...
  /* Configure SPI mode */
  ret = ioctl(dev->fd, SPIIOC_SETMODE, dev->mode);
  if (ret < 0)
    {
      _err("ERROR: Failed to set SPI mode: %d\n", errno);
      close(dev->fd);
      return SPI_ERR_IOCTL;
    }

  /* Configure SPI frequency */
  ret = ioctl(dev->fd, SPIIOC_SETFREQUENCY, dev->frequency);
  if (ret < 0)
    {
      _err("ERROR: Failed to set SPI frequency: %d\n", errno);
      close(dev->fd);
      return SPI_ERR_IOCTL;
    }

  /* Configure bits per word */
  ret = ioctl(dev->fd, SPIIOC_SETBITS, dev->nbits);
  if (ret < 0)
    {
      _err("ERROR: Failed to set SPI bits: %d\n", errno);
      close(dev->fd);
      return SPI_ERR_IOCTL;
    }
  ...
  _info("SPI initialized: %s, devid=%d, freq=%u Hz, mode=%d\n",
        dev->devpath, dev->devid, dev->frequency, dev->mode);
...
int spi_transfer(spi_dev_t *dev, const uint8_t *txbuffer, 
...
int spi_select_bank(spi_dev_t *dev, uint8_t bank)
{
  ...
  return spi_write_reg(dev, REG_BANK_SEL, bank & 0x07);
}