                                  ...
                                  size_t length)
{
  ...
  syslog(LOG_DEBUG, "spi_transfer_internal: Starting SPI transfer\n");

  /* CS assert */
  spi_cs_control(dev, true);
  usleep(SPI_CS_DELAY_US);
  syslog(LOG_DEBUG, "spi_transfer_internal: CS asserted\n");
  ...
  while (retries-- > 0)
    {
      ...
      if (ret >= 0)
        {
          break;
          syslog(LOG_DEBUG, "spi_transfer_internal: SPI transfer succeeded\n");
        }
      usleep(100);  /* Small delay before retry */
    }
  syslog(LOG_DEBUG, "spi_transfer_internal: SPI transfer completed\n");
  /* CS deassert */
  usleep(SPI_CS_DELAY_US);
  spi_cs_control(dev, false);
  syslog(LOG_DEBUG, "spi_transfer_internal: CS deasserted\n");
  syslog(LOG_DEBUG, "spi_transfer_internal: SPI transfer result: %d\n", ret);
                  ...
                  uint8_t *buffer, size_t length)
{
  ...
  syslog(LOG_DEBUG, "spi_read_regs: reg_addr=0x%02X, length=%zu\n", 
         reg_addr, length);
  ...
  syslog(LOG_DEBUG, "spi_read_regs: Buffers allocated\n");
  ...
  syslog(LOG_DEBUG, "spi_read_regs: Starting SPI transfer\n");
  ...
  if (ret == SPI_OK)
    ...
    {
      syslog(LOG_ERR, "ERROR: spi_transfer_internal failed in spi_read_regs: %d\n", ret);
    }
  syslog(LOG_INFO, "spi_read_regs: Data read successfully\n");
  free(tx);
  free(rx);
  return ret;
}
