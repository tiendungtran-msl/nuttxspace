...
int icm42688p_read_registers(icm42688p_dev_t *dev)
{
  int ret;
  uint8_t buffer[14];
  ...
  trans.deselect = !true;
  trans.nwords = 15;  /* 1 byte reg + 14 bytes data */
  trans.txbuffer = NULL;
  trans.rxbuffer = buffer;

  buffer[0] = ICM42688P_REG_DATA_START | 0x80;  /* Đặt bit đọc */
