...
int icm42688p_get_data(icm42688p_dev_t *dev)
{
  ...
  ret = icm42688p_read_registers(dev, ICM42688P_TEMP_DATA1, (uint8_t *)data, 14);
  if (ret != ICM42688P_OK)
    {
...
int icm42688p_self_test(icm42688p_dev_t *dev)
{
  ...
  if (dev->temperature < -40.0f || dev->temperature > 85.0f)
    {
...
int icm42688p_orientation(icm42688p_dev_t *dev, float *pitch, float *roll)
{
  ...
  *pitch = atan2f(-dev->accel.x, sqrtf(dev->accel.y * dev->accel.y + dev->accel.z * dev->accel.z)) * (180.0f / M_PI);
  *roll  = atan2f(dev->accel.y, dev->accel.z) * (180.0f / M_PI);
