...
static int icm42688p_select_bank(icm42688p_dev_t *dev, uint8_t bank)
{
  ...
  ret = spi_write_reg(&dev->spi, ICM42688P_REG_BANK_SEL, bank & 0x07);
  if (ret != SPI_OK)
    {
      _err("ERROR: Failed to select bank %d\n", bank);
...
static int icm42688p_configure(icm42688p_dev_t *dev)
{
  ...
  if (ret != SPI_OK)
    {
      _err("ERROR: Failed to configure power management\n");
      return ICM42688P_ERR_COMM;
    }
  ...
  if (ret != SPI_OK)
    {
      _err("ERROR: Failed to configure gyroscope\n");
      return ICM42688P_ERR_COMM;
    }
  ...
  if (ret != SPI_OK)
    {
      _err("ERROR: Failed to configure accelerometer\n");
      return ICM42688P_ERR_COMM;
    }
  ...
  if (ret != SPI_OK)
    {
      _err("ERROR: Failed to configure filters\n");
      return ICM42688P_ERR_COMM;
    }

  usleep(ICM42688P_POWERUP_DELAY_MS * 1000);  /* Chờ ổn định */

  _info("ICM42688P configured: Gyro ±2000dps, Accel ±16g, ODR 1kHz\n");
...
int icm42688p_init(icm42688p_dev_t *dev, uint8_t bus, uint8_t de)
{
  uint8_t who_am_i;
  int ret;
  ...
  /* Khởi tạo SPI */
  _info("Initializing ICM42688P on SPI%d, CS%d, %u Hz\n",
        bus, devid, frequency);

  ret = spi_init(&dev->spi, bus, devid, frequency, mode);
  if (ret != SPI_OK)
    {
  ...
  /* Đăng ký CS callback nếu có board-specific function */
  if (board_spi1_icm_select != NULL)
    {
      spi_set_cs_callback(&dev->spi, board_spi1_icm_select);
      _info("Registered board-specific CS callback\n");
    }
  ...
  if (ret != ICM42688P_OK)
    {
      _err("ERROR: Failed to select bank 0\n");
      goto errout;
    }
  ...
  if (ret != SPI_OK)
    {
      _err("ERROR: Failed to read WHO_AM_I register\n");
  ...
  if (who_am_i != ICM42688P_WHO_AM_I_VALUE)
    {
      _err("ERROR: Invalid WHO_AM_I: 0x%02X (expected 0x%02X)\n",
           who_am_i, ICM42688P_WHO_AM_I_VALUE);
      ret = ICM42688P_ERR_INVALID;
      goto errout;
    }

  _info("ICM42688P detected, WHO_AM_I = 0x%02X\n", who_am_i);

  /* Soft reset thiết bị */
  ret = icm42688p_reset(dev);
  if (ret != ICM42688P_OK)
    {
      _err("ERROR: Failed to reset device\n");
      goto errout;
    }
  ...
  if (ret != ICM42688P_OK)
    {
      _err("ERROR: Failed to configure device\n");
      goto errout;
    }
...
int icm42688p_reset(icm42688p_dev_t *dev)
{
  ...
  if (ret != SPI_OK)
    {
      _err("ERROR: Soft reset command failed\n");
      return ICM42688P_ERR_COMM;
    }
  ...
  _info("ICM42688P reset complete\n");
...
int icm42688p_read_data(icm42688p_dev_t *dev, icm42688p_data_t *data)
{
  ...
  if (ret != SPI_OK)
    {
      dev->error_count++;
      _err("ERROR: Failed to read sensor data\n");
      return ICM42688P_ERR_COMM;
    }
...
int icm42688p_calibrate_gyro(icm42688p_dev_t *dev, int num_samples)
{
  ...
  _info("Starting gyro calibration with %d samples...\n", num_samples);
  _info("*** KEEP DEVICE STATIONARY! ***\n");
  ...
  for (i = 0; i < num_samples; i++)
    {
      ...
      if (ret != ICM42688P_OK)
        {
          _err("ERROR: Calibration read failed at sample %d\n", i);
          return ret;
        }
  ...
  _info("  Offset X: %.3f dps\n", dev->gyro_offset.x);
  _info("  Offset Y: %.3f dps\n", dev->gyro_offset.y);
  _info("  Offset Z: %.3f dps\n", dev->gyro_offset.z);
...
int icm42688p_self_test(icm42688p_dev_t *dev)
{
  ...
  _info("Running ICM42688P self-test...\n");
  ...
  if (accel_magnitude < 0.5f || accel_magnitude > 1.5f)
    {
      _err("ERROR: Accelerometer magnitude out of range: %.2f g\n",
           accel_magnitude);
  ...
  if (gyro_magnitude > 50.0f)  /* > 50 dps khi đứng yên là bất thường */
    {
      _warn("WARNING: Gyro magnitude high: %.2f dps (may need calibration)\n",
            gyro_magnitude);
    }
  ...
  if (data.temperature < -40.0f || data.temperature > 85.0f)
    {
      _err("ERROR: Temperature out of range: %.2f °C\n", data.temperature);
      return ICM42688P_ERR_SELFTEST;
    }

  _info("Self-test PASSED\n");
  _info("  Accel magnitude: %.3f g\n", accel_magnitude);
  _info("  Gyro magnitude:  %.2f dps\n", gyro_magnitude);
  _info("  Temperature:     %.2f °C\n", data.temperature);
...
int icm42688p_set_gyro_range(icm42688p_dev_t *dev, uint8_t range)
{
  ...
  _info("Gyro range set to %d (sensitivity: %.1f LSB/dps)\n",
        range, sensitivity);
...
int icm42688p_set_accel_range(icm42688p_dev_t *dev, uint8_t range)
{
  ...
  _info("Accel range set to %d (sensitivity: %.1f LSB/g)\n",
        range, sensitivity);
