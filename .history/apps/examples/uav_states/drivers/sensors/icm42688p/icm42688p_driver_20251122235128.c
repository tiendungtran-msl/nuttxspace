...
static int icm42688p_spi_write(icm42688p_dev_t *dev, uint8_t reg, uint8_t value)
{
  int ret;
  ...
  seq.mode = ICM42688P_SPI_MODE;
  seq.nbits = 8;
  seq.ntrans = 1;
  seq.frequency = ICM42688P_SPI_FREQUENCY;
  seq.trans = &trans;

  /* Perform transfer with retry */
  for (int retry = 0; retry < ICM42688P_SPI_RETRY_COUNT; retry++)
    {
...
static int icm42688p_spi_read(icm42688p_dev_t *dev, uint8_t reg, uint8_t *value)
{
  int ret;
...
/* Low-level SPI burst read */

static int icm42688p_spi_read_burst(icm42688p_dev_t *dev, uint8_t reg, uint8_t *buffer, size_t length)
{
  ...
  seq.nbits = 8;
  seq.ntrans = 1;
  seq.frequency = ICM42688P_SPI_FREQUENCY;  /* 10 MHz */
  seq.trans = &trans;
      ...
      if (retry < ICM42688P_SPI_RETRY_COUNT - 1)
        {
          syslog(LOG_WARNING, "[ICM%d] SPI burst read retry %d/%d (ret=%d)\n", dev->id,
                retry + 1, ICM42688P_SPI_RETRY_COUNT, ret);
          usleep(ICM42688P_SPI_RETRY_DELAY_US);
        }
    }
...
static int icm42688p_verify_register(icm42688p_dev_t *dev, uint8_t reg, uint8_t value)
{
  int ret;
  uint8_t read_value;
...
static int icm42688p_configure(icm42688p_dev_t *dev)
{
  ...
  ret = icm42688p_verify_register(dev, ICM42688P_PWR_MGMT0, PWR_MGMT0_GYRO_MODE_LN | PWR_MGMT0_ACCEL_MODE_LN);
  if (ret != ICM42688P_OK)
    {
  ...
  ret = icm42688p_verify_register(dev, ICM42688P_GYRO_CONFIG0, dev->gyro_fs | GYRO_CONFIG0_ODR_1KHZ);
  if (ret != ICM42688P_OK)
    {
  ...
  ret = icm42688p_verify_register(dev, ICM42688P_ACCEL_CONFIG0, dev->accel_fs | ACCEL_CONFIG0_ODR_1KHZ);
  if (ret != ICM42688P_OK)
    {
...
int icm42688p_orientation(icm42688p_dev_t *dev)
{
  ...
  dev->pitch = atan2f(-dev->accel.x,sqrtf(dev->accel.y * dev->accel.y + 
