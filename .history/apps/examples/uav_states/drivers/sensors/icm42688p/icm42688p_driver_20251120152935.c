...
int icm42688p_write_register(icm42688p_dev_t *dev, uint8_t reg, uint8_t value)
{
  ...
  struct spi_dev_s icm_dev;
  icm_dev = board_spi1_icm_get_device(dev->id);
  if (icm_dev == NULL)
    {
...
int icm42688p_init(icm42688p_dev_t *dev)
{
  int ret;
...
int icm42688p_get_data(icm42688p_dev_t *dev, icm42688p_data_t *data)
{
  int ret;
  ...
  ret = icm42688p_read_registers(dev, ICM42688P_TEMP_DATA1, (uint8_t *)data, sizeof(icm42688p_data_t));
  if (ret != ICM42688P_OK)
    {
  ...
  dev->temp     = (int16_t)((buffer[0] << 8)  | buffer[1]);
  dev->accel.x  = (int16_t)((buffer[2] << 8)  | buffer[3]);
  dev->accel.y  = (int16_t)((buffer[4] << 8)  | buffer[5]);
  dev->accel.z  = (int16_t)((buffer[6] << 8)  | buffer[7]);
  dev->gyro.x   = (int16_t)((buffer[8] << 8)  | buffer[9]);
  dev->gyro.y   = (int16_t)((buffer[10] << 8) | buffer[11]);
  dev->gyro.z   = (int16_t)((buffer[12] << 8) | buffer[13]);
  
  /* Chuyển đổi dữ liệu raw sang giá trị thực */
  dev->accel.x = (float)raw_accel_x / dev->accel_sensitivity;
  dev->accel.y = (float)raw_accel_y / dev->accel_sensitivity;
  dev->accel.z = (float)raw_accel_z / dev->accel_sensitivity;

  dev->gyro.x = ((float)raw_gyro_x / dev->gyro_sensitivity) - dev->gyro_offset.x;
  dev->gyro.y = ((float)raw_gyro_y / dev->gyro_sensitivity) - dev->gyro_offset.y;
  dev->gyro.z = ((float)raw_gyro_z / dev->gyro_sensitivity) - dev->gyro_offset.z;
