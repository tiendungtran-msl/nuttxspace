...
static int bmm150_write_reg(bmm150_dev_t *dev, uint8_t reg, uint8_t value)
{
  ...
  config.address = BMM150_I2C_ADDRESS_CSB_LOW_SDO_HIGH; /* Default I2C address */
  config.addrlen = 7; /* 7-bit address */

  /* Configure I2C device */
  ret = i2c_configure(&dev->i2c, &config);
  if (ret < 0)
    {
  ...
  if (i2c_write(&dev->i2c, buffer, 2) < 0)
    {
      dev->error_count++;
      return BMM150_ERR_COMM;
    }
