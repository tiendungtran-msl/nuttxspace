...
static int bmm150_soft_reset(bmm150_dev_t *dev)
{
  ...
  up_msleep(BMM150_DELAY_SOFT_RESET); /* Chờ reset hoàn tất */
      ...
      if ((dev->trim.dig_z2 != 0) && (dev->trim.dig_z1 != 0) &&
          (data_rhall != 0) && (dev->trim.dig_xyz1 != 0))
        {
          ...
          process_comp_z3 = (((float)dev->trim.dig_z2) *
                            (process_comp_z0 * process_comp_z0 / 268435456.0f));
...
int bmm150_init(bmm150_dev_t *dev, uint8_t addr)
{
  ...
  /* 3. Power on */
  ret = bmm150_write_reg(dev, BMM150_REG_POWER_CONTROL, BMM150_POWER_CNTRL_ENABLE);
  if (ret != BMM150_OK)
    {
...
int bmm150_read_mag(bmm150_dev_t *dev)
{
  ...
  mag_data->x = bmm150_compensate_x(dev, raw_data.raw_x, raw_data.raw_rhall);
  mag_data->y = bmm150_compensate_y(dev, raw_data.raw_y, raw_data.raw_rhall);
  mag_data->z = bmm150_compensate_z(dev, raw_data.raw_z, raw_data.raw_rhall);
...
int bmm150_get_heading(bmm150_dev_t *dev)
{
  ...
  dev->yaw = atan2f(mag_data.y, mag_data.x) * 180.0f / M_PI;
