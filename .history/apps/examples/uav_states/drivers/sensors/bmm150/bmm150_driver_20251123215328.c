...
int bmm150_init(bmm150_dev_t *dev, uint8_t addr)
{
  ...
  if (ret != BMM150_OK)
    {
      sys("BMM150: Soft reset failed\n");
      goto error;
    }
  up_mdelay(BMM150_START_UP_TIME);

  /* Read and verify chip address */
  ret = bmm150_read_reg(dev, addr, &who_am_i);
  if (ret != BMM150_OK)
    {
      printf("BMM150: Failed to read chip address\n");
      goto error;
    }

  if (who_am_i != addr)
    {
      printf("BMM150: Invalid chip address: 0x%02X (expected 0x%02X)\n", who_am_i, addr);
      ret = BMM150_ERR_INVALID;
      goto error;
    }
  printf("BMM150: Chip address verified: 0x%02X\n", who_am_i);
