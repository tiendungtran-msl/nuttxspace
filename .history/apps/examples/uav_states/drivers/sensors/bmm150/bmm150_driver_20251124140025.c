...
static int bmm150_set_power_mode(bmm150_dev_t *dev, uint8_t mode)
{
  ...
  ret = bmm150_read_reg(dev, BMM150_REG_POWER_CONTROL, &reg_data);
  if (ret != BMM150_OK)
    {
      return ret;
    }

  reg_data = BMM150_SET_BITS(reg_data, BMM150_POWER_CONTROL, mode);

  ret = bmm150_write_reg(dev, BMM150_REG_POWER_CONTROL, reg_data);
  if (ret != BMM150_OK)
    {
      return ret;
    }

  usleep(BMM150_START_UP_TIME * 1000); /* Chờ chế độ nguồn điện ổn định */
  return BMM150_OK;
}
