...
ret = icm42688p_init(&dev,
  ...
  if (ret != ICM42688P_OK)
    {
      /* IN RÕ MÃ LỖI ĐỂ BIẾT CHÍNH XÁC */
      fprintf(stderr, "\nERROR: ICM-42688-P initialization FAILED! (ret = %d)\n", ret);
      
      switch (ret)
        {
          case ICM42688P_ERR_NODEV:
            fprintf(stderr, " → Chip không trả lời (WHOAMI failed). Kiểm tra:\n");
            fprintf(stderr, "    • Dây nối SPI (SCK, MOSI, MISO, CS)\n");
            fprintf(stderr, "    • Điện áp 3.3V có vào VDDIO & VDD không\n");
            fprintf(stderr, "    • CS pin có đúng không (thường GPIO)\n");
            break;
          case ICM42688P_ERR_SPI:
            fprintf(stderr, " → Lỗi giao tiếp SPI (exchange failed)\n");
            break;
          case ICM42688P_ERR_INVALID:
            fprintf(stderr, " → WHOAMI trả về giá trị sai (chip khác hoặc hỏng)\n");
            break;
          default:
            fprintf(stderr, " → Lỗi không xác định\n");
            break;
        }
      fprintf(stderr, "\n");
      return EXIT_FAILURE;
