...
static int mpu6050_read_reg(int fd, uint8_t reg, uint8_t *data, uint8_t addr)
{
    ...
    /* Cấu hình message 1: Gửi địa chỉ thanh ghi */
    msg[0].frequency = 400000;  /* Thử 400kHz trước */
    msg[0].addr      = addr;
    msg[0].flags     = 0;
    msg[0].buffer    = &reg;
    msg[0].length    = 1;

    /* Cấu hình message 2: Đọc dữ liệu */
    msg[1].frequency = 400000;
    msg[1].addr      = addr;
    msg[1].flags     = I2C_M_READ | I2C_M_NOSTART; /* Repeated start */
    msg[1].buffer    = data;
    msg[1].length    = 1;
    ...
    if (ret < 0)
    {
        ...
        /* Thử giảm tần số xuống 100kHz nếu thất bại */
