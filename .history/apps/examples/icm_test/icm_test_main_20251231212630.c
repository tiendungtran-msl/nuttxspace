...
extern FAR struct spi_dev_s *stm32_spibus_initialize(int bus);
/****************************************************************************
  ...
  if (g_spi1 == NULL)
    {
      ...
      if (g_spi1 == NULL)
        {
          printf("ERROR: Failed to initialize SPI bus %d\n", ICM42688_SPI_BUS);
          return EXIT_FAILURE;
        }
    }

  printf("  %s: SPI bus initialized\n", name);
  ...
  for (i = 0; i < NUM_IMUS; i++)
    {
      ...
      printf("%s: %d reads in %lu ms (%.1f Hz)\n",
             g_imu_names[i],
             iterations,
             elapsed_ms,
