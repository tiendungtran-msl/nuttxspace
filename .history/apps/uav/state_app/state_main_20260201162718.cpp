...
static void publish_state(uint64_t timestamp)
{
    ...
    g_state_msg.arming_state = g_armed
        ? static_cast<uint8_t>(ArmingState::ARMED)
        : static_cast<uint8_t>(ArmingState::DISARMED);

    // Map internal state to message mode
    switch (g_uav_state) {
        case UavState::UNINITIALIZED:
            g_state_msg.mode = static_cast<uint8_t>(VehicleMode::INIT);
            break;
        case UavState::STANDBY:
            g_state_msg.mode = static_cast<uint8_t>(VehicleMode::PREFLIGHT);
            break;
        case UavState::ARMED:
            g_state_msg.mode = static_cast<uint8_t>(VehicleMode::ARMED);
            break;
        case UavState::IN_FLIGHT:
            g_state_msg.mode = static_cast<uint8_t>(VehicleMode::FLIGHT);
            break;
        case UavState::EMERGENCY:
            g_state_msg.mode = static_cast<uint8_t>(VehicleMode::FAILSAFE);
            break;
    }
...
static int state_thread_main(int argc, char *argv[])
{
    ...
    if (g_led_fd < 0) {
        ...
        g_led_on = false;
        g_led_next_toggle = uav_time_us() + LED_BLINK_PERIOD_US;
    }
    ...
    while (!g_should_exit) {
        uint64_t loop_start = uav_time_us();
        ...
        uint64_t elapsed = uav_time_us() - loop_start;
