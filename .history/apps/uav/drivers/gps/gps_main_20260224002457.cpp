...
/****************************************************************************
 * LED Control – PA14 (GPIO_USER_LED defined in board.h)
 *
 * PA14 is the SWCLK pin.  Reconfiguring it as GPIO OUTPUT will disable
 * SWD/JTAG debugging until the next power-on reset.  The pin is only
 * touched when the user explicitly runs "gps_app led init".
 *
 * Direct register access is used (flat build, same address space).
 ****************************************************************************/

/* STM32H7 GPIOA peripheral base */
...
static volatile bool g_gps_led_active = false;

static void gps_led_init(void)
{
    volatile uint32_t *moder =
        (volatile uint32_t *)(GPIOA_BASE_ADDR + GPIO_MODER_OFFSET);
    volatile uint32_t *otyper =
        (volatile uint32_t *)(GPIOA_BASE_ADDR + GPIO_OTYPER_OFFSET);
    volatile uint32_t *ospeedr =
        (volatile uint32_t *)(GPIOA_BASE_ADDR + GPIO_OSPEEDR_OFFSET);

    /* MODER14 = 01 (General-purpose output) */
    uint32_t m = *moder;
    m &= ~(3U << (PA14_BIT * 2));
    m |=  (1U << (PA14_BIT * 2));
    *moder = m;
    ...
    *ospeedr = s;
    ...
    volatile uint32_t *bsrr =
        (volatile uint32_t *)(GPIOA_BASE_ADDR + GPIO_BSRR_OFFSET);
    *bsrr = (1U << (PA14_BIT + 16));

    g_gps_led_active = true;
    printf("[GPS] LED PA14 initialised (WARNING: SWD disabled until reset)\n");
}

static void gps_led_set(bool on)
{
    if (!g_gps_led_active)
    {
        return;
    ...
    if (on)
    {
        *bsrr = (1U << PA14_BIT);          /* set */
    }
    else
    {
        *bsrr = (1U << (PA14_BIT + 16));   /* reset */
    }
}

static void gps_led_blink(int count)
{
    if (!g_gps_led_active)
    {
        gps_led_init();
...
static int gps_selftest(int wait_seconds)
{
    if (wait_seconds < 1)
    {
        wait_seconds = 1;
    ...
    if (g_gps_running)
    {
        ...
        if (!g_gps_driver)
        {
            printf("[SELFTEST] FAIL: driver thread running but instance not ready\n");
            printf("====================\n\n");
            return -EIO;
        }

        uint32_t start_msg_count = g_gps_driver->getMessageCount();
        uint32_t err_count = g_gps_driver->getErrorCount();
        bool got_fix = false;

        printf("[SELFTEST] UART: %s\n", g_gps_active_uart ? g_gps_active_uart : "none");
        printf("[SELFTEST] Waiting up to %d s for GNSS fix...\n", wait_seconds);

        for (int sec = 0; sec < wait_seconds; sec++)
        {
            const gps_data_s *data = g_gps_driver->getData();
            uint32_t msg_count = g_gps_driver->getMessageCount();
            uint8_t fix_type = data->fix_type;
            uint8_t num_sats = data->num_sats;

            printf("[SELFTEST] t=%2ds msgs=%lu fix=%u sats=%u\n",
                   sec + 1,
        ...
        uint32_t end_msg_count = g_gps_driver->getMessageCount();
        err_count = g_gps_driver->getErrorCount();

        printf("[SELFTEST] UBX messages: start=%lu end=%lu delta=%lu, parse errors=%lu\n",
               (unsigned long)start_msg_count,
               (unsigned long)end_msg_count,
               (unsigned long)(end_msg_count - start_msg_count),
               (unsigned long)err_count);

        if (end_msg_count == 0)
        {
            printf("[SELFTEST] FAIL: no UBX messages received\n");
            printf("[SELFTEST] Check wiring/power/UART port selection\n");
            printf("====================\n\n");
            return -EIO;
        }
        ...
        {
            printf("[SELFTEST] WARN: no new UBX frames during wait window\n");
        }

        if (err_count > (end_msg_count / 2U + 10U))
        {
            printf("[SELFTEST] WARN: high parse error ratio\n");
            printf("[SELFTEST] Check baudrate mismatch or line noise\n");
        }

        if (!got_fix)
        {
            printf("[SELFTEST] LINK OK: UART/UBX communication is working\n");
            printf("[SELFTEST] GNSS NO-FIX after %d s: likely RF/antenna/sky-view/power issue\n", wait_seconds);
            printf("[SELFTEST] Try open-sky test for 2-5 minutes (cold start can be slow)\n");
            printf("====================\n\n");
            return -EAGAIN;
        }

        printf("[SELFTEST] PASS: valid GNSS fix available\n");
        printf("====================\n\n");
        return 0;
    }
...
/**
 * @brief LED sub-command handler
 */
static int gps_led_command(int argc, char *argv[])
{
    if (argc < 3)
    {
        printf("Usage: gps_app led <init|on|off|blink [N]>\n");
        printf("  init     - Configure PA14 as output (disables SWD!)\n");
        printf("  on       - Turn LED on\n");
        printf("  off      - Turn LED off\n");
        printf("  blink [N]- Blink N times (default 3)\n");
        return 0;
    }
    ...
    {
        printf("[GPS] WARNING: PA14 is SWCLK – SWD debugging will be "
               "disabled until next reset!\n");
        gps_led_init();
        /* Confirm init with 2 quick blinks */
        gps_led_blink(2);
        return 0;
    }
    ...
    if (!g_gps_led_active)
    {
        printf("[GPS] LED not initialised. Run 'gps_app led init' first.\n");
        return -EINVAL;
    }
    ...
    else if (strcmp(sub, "blink") == 0)
    {
        ...
        printf("[GPS] Blinking %d times...\n", count);
        gps_led_blink(count);
        printf("[GPS] Done\n");
    }
    else
    {
        printf("Unknown LED sub-command: %s\n", sub);
    }
...
/**
 * @brief Detailed GPS diagnostics
 */
static void gps_diag(void)
{
    printf("\n=== GPS Diagnostics ===\n");
    printf("Running:    %s\n", g_gps_running ? "YES" : "NO");
    printf("Init state: %s\n", gps_init_state_str(g_gps_init_state));
    printf("UART:       %s\n", g_gps_active_uart ? g_gps_active_uart : "none");
    printf("LED PA14:   %s\n", g_gps_led_active ? "active" : "off");
    printf("Last error: %d\n", g_gps_last_error);
    ...
    if (g_gps_driver)
    {
        ...
        const gps_data_s *d = g_gps_driver->getData();
        printf("\nNAV-PVT fields:\n");
        printf("  fix_type:       %u\n", d->fix_type);
        printf("  num_sats:       %u\n", d->num_sats);
        printf("  position_valid: %s\n", d->position_valid ? "YES" : "NO");
        printf("  velocity_valid: %s\n", d->velocity_valid ? "YES" : "NO");
        printf("  time_valid:     %s\n", d->time_valid ? "YES" : "NO");
        ...
        printf("  hacc: ");
        if (isfinite(d->hacc)) printf("%.2f m\n", d->hacc);
        else printf("N/A\n");

        printf("  vacc: ");
        if (isfinite(d->vacc)) printf("%.2f m\n", d->vacc);
        else printf("N/A\n");
        ...
        if (msgs == 0)
        {
            printf("  [!] No UBX messages – UART/wiring/power problem\n");
            printf("      Check: GPS VCC, TX→MCU_RX, correct /dev/ttySx\n");
        }
        else if (d->fix_type == 0 && d->num_sats == 0)
        {
            printf("  [!] UBX link OK but ZERO satellites seen\n");
            printf("      Likely causes:\n");
            printf("        - Antenna not connected / broken\n");
            printf("        - Active antenna needs V_ANT power\n");
        ...
        {
            printf("  [~] Tracking %u sats but no fix yet\n", d->num_sats);
            printf("      Need >=4 sats for 3D fix. Wait longer.\n");
        }
        else
        {
            printf("  [OK] Fix=%u, Sats=%u\n", d->fix_type, d->num_sats);
        }

        if (errs > 0 && errs > msgs / 2)
        {
            printf("  [!] High parse error ratio (%lu/%lu) – "
                   "baudrate mismatch or noise\n",
                   (unsigned long)errs, (unsigned long)msgs);
        }
    }
    else
    {
        printf("Driver instance: NULL\n");
    }
...
static void *gps_thread_main(void *arg)
{
    ...
    while (!g_gps_should_stop)
    {
        ...
        if (ret > 0)
        {
            ...
            /* Brief LED pulse on each UBX message (if LED initialised) */
            if (g_gps_led_active)
            {
                gps_led_set(true);
            }
