...
#define UBX_MSG_MON_RF              ((UBX_CLASS_MON << 8) | UBX_ID_MON_RF)
...
#define UBX_ACK_TIMEOUT_SLOW_MS     2000
...
/* MON-RF message header */
struct ubx_mon_rf_hdr_s
{
    uint8_t version;
    uint8_t nBlocks;
    uint8_t reserved[2];
...
/* MON-RF block (24 bytes each) */
struct ubx_mon_rf_block_s
{
    uint8_t  blockId;
    uint8_t  flags;
    uint8_t  antStatus;
...
struct gps_data_s
{
    ...
    /* Receiver diagnostics (from MON-VER / MON-RF) */
    bool     mon_ver_valid;
    bool     mon_rf_valid;
    bool     proto_ver_27_or_higher;
    uint8_t  hw_generation;   /* 0=unknown, 8/9/10... */
    uint16_t proto_major;
    uint16_t proto_minor;
    char     module_name[16];

    uint8_t  rf_blocks;
    uint8_t  rf_ant_status;
    uint8_t  rf_ant_power;
    uint8_t  rf_jam_ind;
...
public:
    ...
    bool isProtoVer27OrHigher() const { return _proto_ver_27_or_higher; }

    uint8_t getBoardGeneration() const { return _board_generation; }
    ...
    /**
     * @brief Check if PPS signal is available
     * @return true if PPS detected
     */
    bool hasPPS() const { return _pps_detected; }

    /**
     * @brief Get last PPS timestamp
     */
    uint64_t getPPSTimestamp() const { return _pps_timestamp_us; }
