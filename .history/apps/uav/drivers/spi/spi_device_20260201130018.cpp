/****************************************************************************
 * apps/uav/drivers/spi/spi_device.cpp
 *
 * SPI device base class implementation
 ****************************************************************************/
...
#include <uav/lib/platform/spi_config.h>
}
...
extern "C" {
/* Low-level STM32 SPI init (fallback, thường đã được board init) */
struct spi_dev_s *stm32_spibus_initialize(int bus);

/* Board-level API cho ICM42688P SPI devices (preferred) */
struct spi_dev_s *board_spi1_icm_get_device(uint8_t id);
void board_spi1_icm_select(uint8_t id, bool selected);
}

#ifndef CONFIG_SPI_EXCHANGE
# warning "Driver hoạt động tốt nhất với CONFIG_SPI_EXCHANGE"
#endif
...
namespace drivers {
    namespace spi {
        ...
        Device::~Device()
        {
            /* NuttX không cung cấp SPI de-initialization */
            _dev = nullptr;
        }
        ...
        int Device::init()
        {
            ...
            /* Với SPI1 + ICM sensors, dùng board-level wrapper xử lý CS pin */
            if (_config.bus == 1) {
                /* Convert devid sang sensor index: SPIDEV_IMU(1)=1 -> id=0, etc. */
                uint8_t sensor_id = _config.devid - 1;
                if (sensor_id > 3) {
                    DRIVER_ERR("Invalid ICM sensor ID %d (devid=%d)",
                               sensor_id, (int)_config.devid);
                    return -EINVAL;
                }

                _dev = board_spi1_icm_get_device(sensor_id);

                if (_dev == nullptr) {
                    DRIVER_ERR("Failed to get SPI1 ICM wrapper for sensor %d", sensor_id);
                    return -ENODEV;
                }

                DRIVER_INFO("Using board SPI1 ICM wrapper for sensor %d", sensor_id);
            } else {
                /* Với các SPI bus khác, dùng standard initialization */
                if (!board_has_spi_bus(_config.bus)) {
                    DRIVER_WARN("SPI bus %d not available", _config.bus);
                }
            ...
            /* Deselect device (đảm bảo high-to-low transition ở lần select tiếp) */
...
#else
            /* Fallback: send và receive riêng */
            if (send && recv) {
                for (size_t i = 0; i < len; i++) {
                    recv[i] = SPI_SEND(_dev, send[i]);
                }
            } else if (send) {
                SPI_SNDBLOCK(_dev, send, len);
            } else if (recv) {
                SPI_RECVBLOCK(_dev, recv, len);
            }
#endif
        ...
        void Device::_lock()
        {
            /* Không lock trong interrupt context */
