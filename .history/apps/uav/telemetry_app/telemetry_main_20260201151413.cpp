...
static void fill_packet_data(TelemetryContext *ctx)
{
    ...
    if (ctx->combined_sub >= 0)
    {
        ...
        if (updated && uorb::orb_copy(ctx->combined_sub, &combined) == 0)
        {
    ...
    if (ctx->status_sub >= 0)
    {
        ...
        if (updated && uorb::orb_copy(ctx->status_sub, &status) == 0)
        {
...
static int telemetry_thread_main(int argc, char *argv[])
{
    ...
    ctx->combined_sub = uorb::orb_subscribe(uorb::sensor_combined);
    if (ctx->combined_sub < 0)
    {
        syslog(LOG_WARNING, "[telemetry] Failed to subscribe sensor_combined\n");
    }

    ctx->status_sub = uorb::orb_subscribe(uorb::system_status);
    if (ctx->status_sub < 0)
    {
