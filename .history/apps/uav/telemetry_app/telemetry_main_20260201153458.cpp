...
static void fill_packet_data(TelemetryContext *ctx)
{
    ...
    if (ctx->imu_sub >= 0)
    {
        ...
        if (updated && uorb::orb_copy(ORB_ID(sensor_imu), ctx->imu_sub, &imu) == 0)
        {
            pkt->imu.temperature = imu.temperature;
        }
    }
    ...
    if (ctx->attitude_sub >= 0)
    {
        ...
        if (updated && uorb::orb_copy(ORB_ID(vehicle_attitude), ctx->attitude_sub, &attitude) == 0)
        {
            ...
            /* Angular rates */
            pkt->attitude.rollspeed = attitude.rollspeed;
            pkt->attitude.pitchspeed = attitude.pitchspeed;
            pkt->attitude.yawspeed = attitude.yawspeed;
        }
    }
