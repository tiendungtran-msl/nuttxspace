/****************************************************************************
 * apps/uav/lib/drivers_framework/device_id.hpp
 *
 * Device ID structure cho sensor drivers
 * Định danh duy nhất cho mỗi device trên bus
 ****************************************************************************/

#pragma once

#include <stdint.h>

namespace drivers {

/**
 * @brief Loại bus kết nối device
 */
enum class BusType : uint8_t {
    UNKNOWN = 0,
    I2C     = 1,
    SPI     = 2,
    UART    = 3,
    UAVCAN  = 4,
    SDMMC   = 5
};

/**
 * @brief Device ID structure (32-bit)
 * 
 * Cấu trúc union cho phép truy cập từng field hoặc toàn bộ value.
 */
union DeviceId {
    struct {
        uint8_t devtype;      /**< Device type (sensor specific) */
        uint8_t address;      /**< I2C address hoặc SPI CS index */
        uint8_t bus;          /**< Bus number */
        uint8_t bus_type;     /**< BusType enum */
    } fields;

    uint32_t value;

    DeviceId() : value(0) {}
    explicit DeviceId(uint32_t val) : value(val) {}

    bool is_valid() const { return value != 0; }

    bool operator==(const DeviceId& other) const {
        return value == other.value;
    }

    bool operator!=(const DeviceId& other) const {
        return value != other.value;
    }
};

/**
 * @brief Device type constants
 * 
 * Mỗi loại sensor có một ID duy nhất
 */
namespace DeviceType {
    constexpr uint8_t UNKNOWN       = 0x00;

    /* IMU sensors */
    constexpr uint8_t MPU6050       = 0x09;
    constexpr uint8_t MPU9250       = 0x0A;
    constexpr uint8_t ICM42688P     = 0x0B;
    constexpr uint8_t BMM150        = 0x0C;
    constexpr uint8_t BMI088_ACCEL  = 0x0D;
    constexpr uint8_t BMI088_GYRO   = 0x0E;

    /* Barometer */
    constexpr uint8_t BMP280        = 0x20;
    constexpr uint8_t MS5611        = 0x21;

    /* Magnetometer */
    constexpr uint8_t HMC5883L      = 0x30;
    constexpr uint8_t QMC5883L      = 0x31;
}

} // namespace drivers
