...
// Chia tần số đọc IMU để giảm tải (ví dụ 4 => 1000/4 = 250 Hz mỗi IMU)
#ifndef CONFIG_UAV_IMU_POLL_DIV
#define CONFIG_UAV_IMU_POLL_DIV         4
#endif

// Spin-wait cuối chu kỳ (0 = tắt để giảm CPU)
#ifndef CONFIG_UAV_SPIN_WAIT
#define CONFIG_UAV_SPIN_WAIT            0
#endif
...
static int sensors_thread_main(int argc, char *argv[])
{
    ...
    while (!g_should_exit) {
        ...
        if ((g_loop_count % CONFIG_UAV_IMU_POLL_DIV) == 0) {
            poll_imus(now);
        }
