...
class TelemetrySimulator(QObject):
    ...
    def _run_loop(self):
        ...
        while self._running:
            ...
            # Update stats
            self._stats['bytes_received'] += 212  # Updated for 4 IMUs
    ...
    def _generate_packet(self) -> TelemetryData:
        ...
        # Create packet (field names must match TelemetryData dataclass)
        data = TelemetryData(
            # Header
            ...
            # GPS (convert to proper units)
            latitude=base_lat + 0.0001 * math.sin(0.05 * t),  # degrees
            longitude=base_lon + 0.0001 * math.cos(0.05 * t),  # degrees
            altitude_msl=altitude,  # meters
            ground_speed=5.0,  # m/s
            heading=math.degrees(yaw),  # degrees
            fix_type=3,  # 3D fix
            satellites=12,
            hdop=1.0,
            vdop=1.5,
