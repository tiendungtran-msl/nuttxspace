...
class PacketDecoder:
    ...
    def _decode_buffer(self) -> Optional[TelemetryData]:
        ...
        if data is None:
            self.sync_errors += 1
            # Debug: print first packet decode failure
            if self.sync_errors <= 3:
                print(f"[Decoder] Packet decode failed (sync_err #{self.sync_errors})")
            return None
        
        if not data.crc_valid:
            self.crc_errors += 1
            # Debug: print CRC failures
            if self.crc_errors <= 3:
                print(f"[Decoder] CRC invalid (crc_err #{self.crc_errors})")
            return None
        
        # Debug: print first successful decode
        if self.packets_decoded == 0:
            print(f"[Decoder] First packet OK! seq={data.sequence}, imu_count={len(data.imu)}")
