...
class FastMainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # 3D Attitude view (expanded height to use available space)
        self._attitude_view = AttitudeViewer()
        self._attitude_view.setMinimumHeight(350)  # Taller 3D view
        right_layout.addWidget(self._attitude_view, 2)  # Stretch factor 2
        ...
        right_layout.addWidget(self._sensor_display, 1)  # Stretch factor 1
    ...
    def _setup_connections(self):
        ...
        self._simulator.stats_updated.connect(self._on_stats_updated)
    ...
    def _on_stats_updated(self, stats: dict):
        """Handle stats update from receiver/simulator"""
        self._control_panel.update_stats(stats)
        
        # Update status bar with detailed stats
        crc_err = stats.get('crc_errors', 0)
        sync_err = stats.get('sync_errors', 0)
        if crc_err > 0 or sync_err > 0:
            self._status_drops.setText(f"Drops: {stats.get('sequence_drops', 0)} CRC:{crc_err} Sync:{sync_err}")
            self._status_drops.setStyleSheet("color: #ff6666; font-family: monospace; padding: 0 10px;")
        else:
