/****************************************************************************
 * apps/uav/drivers/imu/icm42688p/icm42688p.cpp
 *
 * ICM-42688-P IMU Driver Implementation
 ****************************************************************************/
...
namespace drivers {
    namespace imu {
        ...
        ICM42688P::ICM42688P(uint8_t bus, uint32_t cs)
            ...
            , _last_read_time(0)
        {
            /* Calibration objects được khởi tạo identity by default */
        }

        /* Legacy API implementation sử dụng calibration objects */
        ...
        {
            /* Apply isotropic scale correction cho all axes */
        ...
        int ICM42688P::initialize()
        {
            ...
            /* Switch to high-speed SPI (24 MHz max, dùng 10 MHz safe) */
            set_frequency(10000000);

            _initialized = true;
            DRIVER_INFO("ICM-42688-P initialized @ %u Hz", (unsigned)get_frequency());
        ...
        int ICM42688P::reset()
        {
            ...
            /* Soft reset (từ PX4: DEVICE_CONFIG, bit 0) */
            ...
            /* Wait for reset to complete (PX4 dùng 1ms) */
            usleep(1000);
        ...
        int ICM42688P::configure()
        {
            ...
            /* 2. Disable AFSR (Adaptive Full Scale Range) - từ PX4 */
            ret = register_write(BANK_0, ICM42688P_INTF_CONFIG1, 0x80);  /* AFSR_CLEAR */
            if (ret != 0) return ret;

            /* 3. Power management: Gyro và Accel trong Low Noise mode */
            ...
            /* Wait for sensors power up */
            usleep(30000);  /* 30ms từ PX4 */
            ...
            /* 6. Configure filters
             * Set UI filter order to 1st order (từ PX4)
             */
            ...
            /* Configure 585Hz AAF for gyro (từ PX4) */
            ret = register_write(BANK_1, 0x0B, 0x0D);  /* GYRO_CONFIG_STATIC3 */
            if (ret != 0) return ret;

            ret = register_write(BANK_1, 0x0C, 0xAA);  /* GYRO_CONFIG_STATIC4 */
            if (ret != 0) return ret;
        ...
        int ICM42688P::read(Data &data)
        {
            ...
            /* Convert to physical units trong sensor frame */
            ...
            /* Apply calibration (bias, scale, rotation) */
