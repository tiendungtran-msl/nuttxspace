...
public:
    ...
    /**
     * @brief Lấy vị trí gốc (GPS origin)
     * @return true nếu đã có origin
     */
    bool getOrigin(double& lat_deg, double& lon_deg, float& alt_m) const
    {
        if (!_origin_set) return false;
        lat_deg = _origin_lat;
        lon_deg = _origin_lon;
        alt_m = _origin_alt;
        return true;
    ...
    /**
     * @brief Kiểm tra filter đã khởi tạo chưa
     */
    bool isInitialized() const { return _filter_initialized; }
